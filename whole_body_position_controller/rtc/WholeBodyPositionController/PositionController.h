...
namespace WholeBodyPosition {
  ...
  public:
    ...
    void control(const std::map<std::string, std::shared_ptr<primitive_motion_level_tools::PrimitiveState> >& primitiveCommandMap, // primitive motion level target
                 const std::vector<std::shared_ptr<WholeBodyPosition::Collision> >& selfCollisions, // current self collision state
                 const std::vector<std::shared_ptr<WholeBodyPosition::Collision> >& envCollisions, // current self collision state
    ...
    const double& comVelocityLimit() const { return comVelocityLimit_;}
    double& comVelocityLimit() { return comVelocityLimit_;}
    const double& angularMomentumLimit() const { return angularMomentumLimit_;}
    double& angularMomentumLimit() { return angularMomentumLimit_;}

  protected:
    class PositionTask {
    public:
      PositionTask(const std::string& name);
      void updateFromPrimitiveCommand(const std::shared_ptr<const primitive_motion_level_tools::PrimitiveState>& primitiveCommand);
  ...
  protected:
    ...
    double comVelocityLimit_ = 0.05;
    double angularMomentumLimit_ = 1.0;
